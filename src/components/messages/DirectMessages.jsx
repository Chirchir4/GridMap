...
function DirectMessages({ setMsgSidebarOpen, selectConversation, messages ,chats}) {
   ...
   const renderAllConversations = () => {
      return chats.map((user) => {
         ...
         return (
            <li className="-mx-2" key={user.id}onClick={() => selectConversation(user.id)}>
               <button className="flex items-center justify-between w-full p-2 rounded" onClick={() => setMsgSidebarOpen(false)}>
                  <div className="flex items-center truncate">
                     ...
                     <div className="truncate">
                        <span className={`text-sm font-medium  ${darkMode ? 'text-slate-400' : 'text-slate-800'}`}>{user.name}</span>
                     </div>
                  </div>
         ...
         <ul className="mb-6">
          ...
          <button className="flex items-center justify-between w-full p-2 rounded bg-indigo-100" onClick={() => setMsgSidebarOpen(false)}>
            <div className="flex items-center truncate">
              ...
              <div className="truncate">
                <span className={`text-sm font-medium  ${darkMode? "text-slate-400":"text-slate-800"}`}>Dominik Lamakani</span>
              </div>
            </div>
            <div className="flex items-center ml-2">
        ...
        <li className="-mx-2">
          <button className="flex items-center justify-between w-full p-2 rounded" onClick={() => setMsgSidebarOpen(false)}>
            <div className="flex items-center truncate">
              ...
              <div className="truncate">
                <span className={`text-sm font-medium  ${darkMode? "text-slate-400":"text-slate-800"}`}>Adrian Przetocki</span>
              </div>
            </div>
            <div className="flex items-center ml-2">
